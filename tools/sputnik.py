...
def BuildOptions():
  ...
  result.add_option("--subtest", default="", help="subtest name")
...
class TestSuite(object):
  ...
  def __init__(self, root, subtest, stric_mode):
    print "root      %s" % root
    print "subtest   %s" % subtest
    self.test_root = path.join(root, subtest)
    print "test_root %s" % self.test_root
    self.lib_root = path.join(root, 'harness')
    print "lib_root  %s" % self.lib_root
...
def Main():
  ...
  test_suite = TestSuite(options.tests, options.subtest, options.enable_strict_mode)
  test_suite.Validate()
  if options.cat:
