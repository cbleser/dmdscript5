 ...
 * )
 * $(TR
 * $(TD $(B $`))    $(TD The portion of d_string that precedes the matched substring.)
 * )
 * $(TR
 * $(TD $(B $'))    $(TD The portion of d_string that follows the matched substring.)
 * )
 * $(TR
...
d_string url = r"(([h|H][t|T]|[f|F])[t|T][p|P]([s|S]?)\:\/\/|~/|/)?([\w]+:\w+@)?(([a-zA-Z]{1}([\w\-]+\.)+([\w]{2,5}))(:[\d]{1,5})?)?((/?\w+/)+|/?)(\w+\.[\w]{3,4})?([,]\w+)*((\?\w+=\w+)?(&\w+=\w+)*([,]\w*)*)?";
...
class RegExpException : Exception
{
    this(d_string msg)
    {
        super(dmdscript.script.fromDstring(msg));
    }
}
...
private alias wchar rchar;   // so we can make a wchar version
...
d_string sub(d_string s, d_string pattern, d_string format, d_string attributes = null)
{
...
d_string sub(d_string s, d_string pattern, d_string delegate(RegExp) dg, d_string attributes = null)
{
    auto r = new RegExp(pattern, attributes);
    ...
    while (r.test(s, lastindex))
    {
        ...
        d_string replacement = dg(r);

        // Optimize by using std.d_string.replace if possible - Dave Fladebo
        d_string slice = result[offset + so .. offset + eo];
...
unittest
{
    ...
    d_string foo(RegExp r) { return "ss"; }

    auto r = sub("hello", "ll", delegate d_string(RegExp r) { return "ss"; });
    assert(r == "hesso");

    r = sub("hello", "l", delegate d_string(RegExp r) { return "l"; }, "g");
    assert(r == "hello");

    auto s = sub("Strap a rocket engine on a chicken.",
            "[ar]",
            delegate d_string (RegExp m)
            {
...
sizediff_t find(d_string s, RegExp pattern)
{
...
sizediff_t
find(d_string s, d_string pattern, d_string attributes = null)
{
    auto r = new RegExp(pattern, attributes);
    scope(exit) delete r;
...
sizediff_t rfind(d_string s, RegExp pattern)
{
...
sizediff_t
rfind(d_string s, d_string pattern, d_string attributes = null)
{
...
d_string[] split(d_string s, RegExp pattern)
{
...
d_string[] split(d_string s, d_string pattern, d_string attributes = null)
{
    auto r = new RegExp(pattern, attributes);
...
RegExp search(d_string s, d_string pattern, d_string attributes = null)
{
    auto r = new RegExp(pattern, attributes);
    if (!r.test(s))
    {   delete r;
...
unittest
{
    ...
    if (auto m = dmdscript.regexp.search("abcdef", "c()"))
    {
    ...
    if (auto n = dmdscript.regexp.search("abcdef", "g"))
    {
        assert(0);
    }
}
...
class RegExp
{
     ...
     * ---
     */
    public this(d_string pattern, d_string attributes = null)
    {
        pmatch = (&gmatch)[0 .. 1];
     ...
     * ---
     */
    public static RegExp opCall(d_string pattern, d_string attributes = null)
    {
    ...
    unittest
    {
        ...
        {
            msg = dmdscript.script.toDstring(ree.toString);
    ...
    public RegExp search(d_string string)
    {
        input = string;
        pmatch[0].rm_eo = 0;
        return this;
     ...
     * null is returned.
     */
    public d_string opIndex(size_t n)
    {
        if (n >= pmatch.length)
            return null;
        else
    ...
    */
    public d_string match(size_t n)
    {
        return this[n];
    }
    ...
    d_string input;       // the string to search

    // per instance:

    d_string pattern;     // source text of the regular expression

    d_string flags;       // source text of the attributes parameter
 ...
 */
    ...
    public void compile(d_string pattern, d_string attributes)
    {
        //printf("RegExp.compile('%.*s', '%.*s')\n", pattern.length, pattern.ptr, attributes.length, attributes.ptr);
        //    std.stdio.writeln("compile pattern=",pattern);
        this.attributes = 0;
 ...
 */
    ...
    public d_string[] split(d_string s)
    {
 ...
 */
    ...
    public sizediff_t find(d_string string)
    {
        if (test(string))
 ...
 */
    ...
    public d_string[] match(d_string s)
    {
        d_string[] result;
 ...
 */
    ...
    public d_string replace(d_string s, d_string format)
    {
        ...
        for (;;)
        {
            ...
            d_string replacement = replace(format);
 ...
 */
    ...
    public d_string[] exec(d_string s)
    {
 ...
 */
    ...
    public d_string[] exec()
    {
        if (!test())
            return null;

        auto result = new d_string[pmatch.length];
 ...
 */
    ...
    public int test(d_string s, size_t startindex)
    {
        char firstc;
        input = s;
    ...
    void printProgram(const(ubyte)[] prog)
    {
        //debug(regexp)
        {
            ...
            for (size_t pc = 0; pc < prog.length; )
            {
                ...
                {
                case REchar:
                    printf("\tREchar '%c' \\x%02X \n", prog[pc + 1], cast(uint)prog[pc+1]);
                    pc += 1 + char.sizeof;
                    break;

                case REichar:
                    printf("\tREichar '%c' \\u%02X \n", prog[pc + 1], cast(uint)prog[pc+1]);
                    pc += 1 + char.sizeof;
                    break;

                case REdchar:
                    printf("\tREdchar '%c' \\u%04X\n", *cast(dchar *)&prog[pc + 1], *cast(ushort *)&prog[pc + 1]);
                    pc += 1 + dchar.sizeof;
                    break;

                case REidchar:
                    printf("\tREidchar '%c' \\u%04X\n", *cast(dchar *)&prog[pc + 1],*cast(ushort *)&prog[pc + 1]);
                    pc += 1 + dchar.sizeof;
                    break;
    ...
    int trymatch(size_t pc, size_t pcend)
    {
        ...
        for (;;)
        {
            ...
            case REdchar:
                ...
                immutable(char)[] str;
                dmdscript.utf.encode(str, *(cast(dchar *)&program[pc + 1]));
                if (!(src+str.length <= input.length && dmdscript.script.toDstring(str) == input[src..src+str.length]))
                    goto Lnomatch;
                src+=str.length;
                pc += 1 + dchar.sizeof;
                break;
        ...
        for (;;)
        {
            ...
            default:
                ...
                if (p >0 && p < pattern.length) {
                    if (pattern[p-1..p+1]=="}{") {
                        error("Invalid regular expression repeated {}{}");
                    }
                 }
                break;
            }
...
//                        writeln("no matching back reference");
                        return 0;
                    }
                    p++;
                    break;
            ...
            Lbyte:
                ...
                if (c >= 0x80)
                {
                    ...
                    auto end=(p+1 <pattern.length)?p+1:pattern.length;
                    op = (op == REchar) ? REdchar : REidchar;
                    buf.write(op);
                    buf.write(cast(wchar)esc);
                }
                else
                {
                  Lchar:
 ...
 */
    ...
    public d_string replace(d_string format)
    {
...
// Static version that doesn't require a RegExp object to be created
    ...
    public static d_string replace3(d_string format, d_string input, regmatch_t[] pmatch)
    {
        d_string result;
        size_t c2;
...
*/
    ...
    public d_string replaceOld(d_string format)
    {
        d_string result;
...
struct Splitter(Range)
{
    ...
    private Range search()
    {
        ...
        auto i = dmdscript.regexp.find(cast(d_string) _input, _rx);
    ...
    this(Range input, Pattern!(wchar) separator)
    {
        _input = input;
...
Splitter!(Range) splitter(Range)(Range r, Pattern!(wchar) pat)
{
    static assert(is(Unqual!(typeof(Range.init[0])) == wchar),
        Unqual!(typeof(Range.init[0])).stringof);
    return typeof(return)(cast(d_string) r, pat);
}

unittest
{
    d_string s1 = ", abc, de,  fg, hi, ";
    auto sp2 = splitter(s1, pattern(", *"w));
    //foreach (e; sp2) writeln("[", e, "]");
    d_string[] expect=["", "abc", "de", "fg", "hi"];

    assert(equal(sp2, expect));
}
...
unittest
{
    ...
    for (wchar c='a'; c<='z'; ++c) {
        d_string re_str= "("w~c~"|)foo"w;
        auto re= new RegExp(re_str);
        bool matches= cast(bool)re.test(str);
        assert(matches);
        //writefln("'%s' matches '%s' ? %s", str, re_str, matches);
    }
}
