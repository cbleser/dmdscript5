...
class DregexpConstructor : Dfunction
{
    ...
    override Value* Construct(CallContext *cc, Value *ret, Value[] arglist)
    {
        ...
        ret.putVundefined();
        pattern = &Value.vundefined;
        flags = &Value.vundefined;
        ...
        if(R)
        {
            if(flags.isUndefined())
            {
                ...
                return RuntimeError(cc.errorInfo, ERR_TYPE_ERROR,
                                    "RegExp.prototype.constructor");
            }
        }
        else
        {
            P = pattern.isUndefined() ? "" : pattern.toText;
            F = flags.isUndefined() ? "" : flags.toText;
        }
        r = new Dregexp(P, F);
        if(r.re.errors)
        {
    ...
    override Value* Put(d_string PropertyName, Value* value, ushort attributes, bool define=false, ushort umask=0)
    {
        return Dfunction.Put(perlAlias(PropertyName), value, attributes, define);
    }

    override Value* Put(d_string PropertyName, Dobject o, ushort attributes, bool define=false)
    {
        return Dfunction.Put(perlAlias(PropertyName), o, attributes, define);
    }

    override Value* Put(d_string PropertyName, d_number n, ushort attributes, bool define=false)
    {
        return Dfunction.Put(perlAlias(PropertyName), n, attributes, define);
    }
    ...
    static d_string perlAlias(d_string s)
    {
        ...
        if(s.length == 2 && s[0] == '$')
        {
            ...
            i = cast(uint)std.string.indexOf(from[], s[1]); // 64bit cast
            if(i >= 0)
                t = to[i];
        }
        return t;
...
Value* Dregexp_prototype_toString(Dobject pthis, CallContext *cc, Dobject othis, Value *ret, Value[] arglist)
{
    ...
    {
        ret.putVundefined();
        return Dobject.RuntimeError(cc.errorInfo, ERR_NOT_TRANSFERRABLE,
...
Value* Dregexp_prototype_test(Dobject pthis, CallContext *cc, Dobject othis, Value *ret, Value[] arglist)
{
...
Value* Dregexp_prototype_exec(Dobject pthis, CallContext *cc, Dobject othis, Value *ret, Value[] arglist)
{
...
Value* Dregexp_prototype_compile(Dobject pthis, CallContext *cc, Dobject othis, Value *ret, Value[] arglist)
{
    ...
    else
    {
        ...
        default:
            attributes = arglist[1].toText;
        case 1:
            pattern = arglist[0].toText;
            break;
        }
...
class DregexpPrototype : Dregexp
{
    this()
    {
        ...
        ushort attributes = ReadOnly | DontDelete | DontEnum;
...
class Dregexp : Dobject
{
    ...
    this(d_string pattern, d_string attributes)
    {
        ...
        Put(TEXT_source, &v, ReadOnly | DontDelete | DontEnum, true);
        Put(TEXT_global, &vb, ReadOnly | DontDelete | DontEnum, true);
        Put(TEXT_ignoreCase, &vb, ReadOnly | DontDelete | DontEnum, true);
        Put(TEXT_multiline, &vb, ReadOnly | DontDelete | DontEnum, true);
        ...
        //  std.stdio.writefln("pattern='%s'",pattern);
        try {
            re = new RegExp(pattern, attributes);
        } catch (RegExpException e) {
            Value err;
            err.putVobject(new syntaxerror.D0(dmdscript.script.toDstring(e.msg)));
            throw new ErrorValue(&err);
        }
        if(re.errors == 0) {
    ...
    static Value* exec(Dobject othis, Value* ret, Value[] arglist, int rettype)
    {
        ...
        {
            ret.putVundefined();
            return RuntimeError(CallContext.currentcc.errorInfo, ERR_NOT_TRANSFERRABLE,
