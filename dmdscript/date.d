 ...
 */
module dmdscript.date;

import std.datetime;
import dmdscript.dateparse;
import dmdscript.datebase;
...
/**
    This translate the old $(D d_time) to $(D SysTime)
...
/**
    This translate the old $(D SysTime) to $(D d_time)

    Returns a $(D d_time) for the given $(D SysTime).
 */
d_time sysTimeToDTime(in SysTime sysTime)
{
 ...
 * ------------------------------------
  ...
  lNow = dmdscript.date.getUTCtime();
  // Convert this into the local date and time for display.
  lNowString = dmdscript.date.UTCtoString(lNow);
 * ------------------------------------
 */
...
d_time toDtime(DosFileTime time)
{
    ...
    t = dmdscript.date.makeDate(dmdscript.date.makeDay(year, month, dayofmonth),
            dmdscript.date.makeTime(hour, minute, second, 0));
