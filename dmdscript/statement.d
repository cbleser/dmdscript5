...
class TopStatement
{
    ...
    this(Loc loc)
    {
        this.loc = loc;
        this.done = 0;
        this.st = d_statement.TOPSTATEMENT;
    }
    ...
    void error(Scope *sc, ...)
    {
        ...
        if(sc.funcdef)
        {
            if(sc.funcdef.isAnonymous)
                ...
                sourcename ~= sc.funcdef.name.toText;
        }
        buf = dmdscript.script.format("%s(%d) : Error: ", sourcename, loc);

        dmdscript.script.sformat(buf, _arguments, _argptr);
        ...
        if(vardecls.length)
        {
            ...
            for(i = 0; i < vardecls.length; i++)
            {
                ...
                if(vd.init)
                {
                    ...
                    property.id = Identifier(vd.name.toText);
                    irs.gen2(loc, IRcode.IRputthis, ret, property.index);
                }
            }
            irs.release(marksave);
...
class BlockStatement : Statement
{
    ...
    override TopStatement ImpliedReturn()
    {
        uint i = cast(uint)statements.length;
...
class LabelStatement : Statement
{
    ...
    override Statement semantic(Scope *sc)
    {
        ...
        if(ls)
        {
            ...
            //if (ls.statement)
            //error(sc, "label '%s' is already defined", ident.toText);
            ls.statement = this;
        }
        else
        {
...
class IfStatement : Statement
{
    ...
    override void toIR(IRstate *irs)
    {
        ...
        u1 = irs.getIP();
        irs.gen2(loc, (condition.isBooleanResult() ? IRcode.IRjfb : IRcode.IRjf), 0, c);
        irs.release(c, 1);
        ifbody.toIR(irs);
        if(elsebody)
        {
    ...
    override void toIR(IRstate *irs)
    {
        ...
        if(cases.length)
        {
            ...
            for(uint i = 0; i < cases.length; i++)
            {
                ...
                cs = cases[i];
                cs.exp.toIR(irs, x);
                irs.gen3(loc, IRcode.IRcid, x, c, x);
                cs.patchIP = irs.getIP();
                irs.gen2(loc, IRcode.IRjt, 0, x);
            }
        }
        udefault = irs.getIP();
        irs.gen1(loc, IRcode.IRjmp, 0);
    ...
    override Statement semantic(Scope *sc)
    {
        ...
        if(sc.switchTarget)
        {
            ...
            for(i = 0; i < sw.cases.length; i++)
            {
                ...
                if(exp == cs.exp)
                {
                    error(sc, errmsgtbl[ERR_SWITCH_REDUNDANT_CASE], exp.toText);
                    return null;
                }
            }
            sw.cases ~= this;
        ...
        {
            error(sc, errmsgtbl[ERR_MISPLACED_SWITCH_CASE], exp.toText);
            return null;
        }
        return this;
    }
...
class DoStatement : Statement
{
    ...
    override void toIR(IRstate *irs)
    {
        ...
        condition.toIR(irs, c);
        irs.gen2(loc, (condition.isBooleanResult() ? IRcode.IRjtb : IRcode.IRjt), u1 - irs.getIP(), c);
        breakIP = irs.getIP();
        irs.release(marksave);
...
class WhileStatement : Statement
{
    ...
    override void toIR(IRstate *irs)
    {
        ...
        irs.gen2(loc, (condition.isBooleanResult() ? IRcode.IRjfb : IRcode.IRjf), 0, c);
        bdy.toIR(irs);
        irs.gen1(loc, IRcode.IRjmp, cast(unsigned)u1 - cast(unsigned)irs.getIP());
        irs.patchJmp(u2, irs.getIP());
        breakIP = irs.getIP();
...
class ForStatement : Statement
{
    ...
    override void toIR(IRstate *irs)
    {
        ...
        if(condition)
        {
            if(condition.op == TOK.TOKless || condition.op == TOK.TOKlessequal)
            {
                ...
                if(be.e2.op == TOK.TOKreal && !isNaN(re.value))
                {
                    u2 = irs.getIP();
					version(D_LP64)
					  irs.gen3(loc, (condition.op == TOK.TOKless) ? IRcode.IRjltc : IRcode.IRjlec, 0, b, re.value);
					else
					  irs.gen(loc, (condition.op == TOK.TOKless) ? IRcode.IRjltc : IRcode.IRjlec, 4, 0, b, re.value);
                }
                else
                {
                    c = irs.alloc(1);
                    be.e2.toIR(irs, c);
                    u2 = irs.getIP();
                    irs.gen3(loc, (condition.op == TOK.TOKless) ? IRcode.IRjlt : IRcode.IRjle, 0, b, c);
                }
            }
            else
            {
...
class ForInStatement : Statement
{
    ...
    override Statement semantic(Scope *sc)
    {
        ...
        if(init.st == d_statement.EXPSTATEMENT)
        {
            ExpStatement es;
        ...
        else if(init.st == d_statement.VARSTATEMENT)
        {
        }
        else
        {
    ...
    override void toIR(IRstate *irs)
    {
        ...
        inexp.toIR(irs, e);
        iter = irs.alloc(1);
        irs.gen2(loc, IRcode.IRiter, iter, e);
        ...
        continueIP = irs.getIP();
        if(opoff == 2)
            irs.gen3(loc, IRcode.IRnextscope, 0, property.index, iter);
        else
            irs.gen(loc, cast(IRcode)(IRcode.IRnext + opoff), 4, 0, base, property.index, iter);
        bdy.toIR(irs);
        irs.gen1(loc, IRcode.IRjmp, cast(unsigned)(continueIP - cast(unsigned)irs.getIP()));
...
class WithStatement : ScopeStatement
{
    ...
    override void toIR(IRstate *irs)
    {
        ...
        c = irs.alloc(1);
        exp.toIR(irs, c);
        irs.gen1(loc, IRcode.IRpush, c);
        bdy.toIR(irs);
        irs.gen0(loc, IRcode.IRpop);
...
class ContinueStatement : Statement
{
    ...
    override Statement semantic(Scope *sc)
    {
        //  writef("ContinueStatement.semantic(%p)\n", sc);
        if(ident == null)
        {
        ...
        else
        {
            ...
            if(!ls || !ls.statement)
            {
                error(sc, errmsgtbl[ERR_UNDEFINED_STATEMENT_LABEL], ident.toText);
                return null;
            }
            else
                target = ls.statement;
    ...
    override void toIR(IRstate *irs)
    {
        ...
        irs.gen1(loc, IRcode.IRjmp, cast(unsigned)cast(void*)this);
    }

    override size_t getTarget()
    {
        assert(target);
...
class BreakStatement : Statement
{
    Identifier *ident;
    Statement target;
    ImpliedReturnStatement implied;

    this(Loc loc, Identifier * ident, Expression exp, bool iseval)
    {
        super(loc);
        this.ident = ident;
        target = null;
        if (iseval) implied=new ImpliedReturnStatement(loc, exp);
    }
        ...
        else
        {
            ...
            else if(!sc.breakTarget)
            {
                ...
                if(ls.statement.whichScope == *sc)
                    error(sc,errmsgtbl[ERR_CANT_BREAK_INTERNAL],ls.ident.toText);
                target = ls.statement;
            }
        }
        return this;
...
class ReturnStatement : Statement
{
    ...
    override void toIR(IRstate *irs)
    {
        ...
        if(exp)
        {
            ...
            if(npops)
            {
                irs.gen1(loc, IRcode.IRimpret, e);
                irs.pops(npops);
                irs.gen0(loc, IRcode.IRret);
            }
            else
                irs.gen1(loc, IRcode.IRretexp, e);
            irs.release(e, 1);
        }
        else
        {
...
class TryStatement : ScopeStatement
{
    ...
    override void toIR(IRstate *irs)
    {
        ...
        {
            f = irs.getIP();
            irs.gen1(loc, IRcode.IRtryfinally, 0);
            if(catchbdy)
            {
                c = irs.getIP();
                irs.gen2(loc, IRcode.IRtrycatch, 0, cast(unsigned)Identifier(catchident.toText));
                bdy.toIR(irs);
                irs.gen0(loc, IRcode.IRpop);           // remove catch clause
                irs.gen0(loc, IRcode.IRpop);           // call finalbdy

                e = irs.getIP();
                irs.gen1(loc, IRcode.IRjmp, 0);
                irs.patchJmp(c, irs.getIP());
                catchbdy.toIR(irs);
                irs.gen0(loc, IRcode.IRpop);           // remove catch object
                irs.gen0(loc, IRcode.IRpop);           // call finalbdy code
                e2 = irs.getIP();
                irs.gen1(loc, IRcode.IRjmp, 0);        // jmp past finalbdy

                irs.patchJmp(f, irs.getIP());
                irs.scopeContext = enclosingScope;
                finalbdy.toIR(irs);
                irs.gen0(loc, IRcode.IRfinallyret);
