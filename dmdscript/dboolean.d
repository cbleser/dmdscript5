...
Value* Dboolean_prototype_toString(Dobject pthis, CallContext *cc, Dobject othis, Value *ret, Value[] arglist)
{
    // othis must be a Boolean
    if(!othis.isClass(TEXT_Boolean)) {
        ret.putVundefined();
        return Dobject.RuntimeError(cc.errorInfo, errmsgtbl[ERR_FUNCTION_WANTS_BOOL],
                                    TEXT_toString,
                                    othis.classname);
    }
    else
...
Value* Dboolean_prototype_valueOf(Dobject pthis, CallContext *cc, Dobject othis, Value *ret, Value[] arglist)
{
