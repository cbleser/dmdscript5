...
private:
    ...
    // to TypeInfo.getHash().
    hash_t getHash(const K key) const
    {
        static if(is (K : const long) && K.sizeof <= hash_t.sizeof)
        {
    ...
    static if(storeHash)
    {
        size_t findExisting(const K key)  const
        {
        ...
        size_t findForInsert(const K key, immutable hash_t hashFull)
        {
            size_t pos = hashFull & mask;
            static if(useRandom)
                size_t rand = hashFull + 1;
    ...
    void assignNoRehashCheck(const K key, ref V val, hash_t hashFull)
    {
...
public:
    ...
    private void reserve(size_t newSize)
    {
        ...
        // !!! It actually escapes !!!
        // So this part is remove we leave to for the GC to cleanup
        //
...
+/
	///Hackery
	V* findExistingAlt(const K key, hash_t hashFull){
            size_t pos = hashFull & mask;
            static if(useRandom)
                size_t rand = hashFull + 1;
            else // static if (P == "perturb")
	...
	void insertAlt(const K key, ref V val, hash_t hashFull){
