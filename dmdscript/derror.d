...
Value* Derror_prototype_toString(Dobject pthis, CallContext *cc, Dobject othis, Value *ret, Value[] arglist)
{
    // ECMA v3 15.11.4.3
    ...
    if(!v)
        v = &Value.vundefined;
    ret.putVstring(othis.Get(TEXT_name).toText()~": "~v.toText());
    return null;
}
...
class Derror : Dobject
{
    this(Value * m, Value * v2)
    {
        ...
        if(v2.isString())
        {
            Put(TEXT_description, v2.toText(), 0);
            Put(TEXT_message, v2.toText(), 0);
        }
        else if(v2.isNumber())
        {
